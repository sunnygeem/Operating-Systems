...
consoleinit(void)
{
  ...
  devsw[CONSOLE].read = consoleread;
    ...
    80000410:	03c78793          	addi	a5,a5,60 # 80022448 <devsw>
...
kfree(void *pa)
{
  ...
  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    ...
    80000a5a:	b8a78793          	addi	a5,a5,-1142 # 800235e0 <end>
...
0000000080000af0 <kinit>:
{
  ...
  freerange(end, (void*)PHYSTOP);
    ...
    80000b14:	ad050513          	addi	a0,a0,-1328 # 800235e0 <end>
    ...
    80000d34:	00c587b3          	add	a5,a1,a2
{
    ...
    80000d3c:	0705                	addi	a4,a4,1 # fffffffffffff001 <end+0xffffffff7ffdba21>
...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  for(int level = 2; level > 0; level--) {
    80000f90:	3a5d                	addiw	s4,s4,-9 # ffffffffffffeff7 <end+0xffffffff7ffdba17>
...
proc_mapstacks(pagetable_t kpgtbl)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    800017ac:	a58a8a93          	addi	s5,s5,-1448 # 80018200 <tickslock>
    char *pa = kalloc();
...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001872:	992a0a13          	addi	s4,s4,-1646 # 80018200 <tickslock>
      initlock(&p->lock, "proc");
...
0000000080001aa2 <allocproc>:
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001ab2:	d5248493          	addi	s1,s1,-686 # 80012800 <proc>
    80001ab6:	00016917          	auipc	s2,0x16
    80001aba:	74a90913          	addi	s2,s2,1866 # 80018200 <tickslock>
    acquire(&p->lock);
...
0000000080001b48 <userinit>:
{
  ...
  initproc = p;
    ...
    80001b5c:	74a7b023          	sd	a0,1856(a5) # 8000a298 <initproc>
  uvmfirst(p->pagetable, initcode, sizeof(initcode));
    80001b60:	03400613          	li	a2,52
    80001b64:	00008597          	auipc	a1,0x8
    80001b68:	6ac58593          	addi	a1,a1,1708 # 8000a210 <initcode>
...
0000000080001c06 <fork>:
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  acquire(&wait_lock);
    ...
    80001cd6:	71648493          	addi	s1,s1,1814 # 800123e8 <wait_lock>
...
0000000080001d14 <scheduler>:
{
  ...
  c->proc = 0;
    ...
    80001d38:	69c70713          	addi	a4,a4,1692 # 800123d0 <pid_lock>
    80001d3c:	975a                	add	a4,a4,s6
    80001d3e:	02073823          	sd	zero,48(a4)
        swtch(&c->context, &p->context);
    80001d42:	00010717          	auipc	a4,0x10
    80001d46:	6c670713          	addi	a4,a4,1734 # 80012408 <cpus+0x8>
    ...
    80001d54:	680a0a13          	addi	s4,s4,1664 # 800123d0 <pid_lock>
    80001d58:	9a3e                	add	s4,s4,a5
        found = 1;
    80001d5a:	4b85                	li	s7,1
    for(p = proc; p < &proc[NPROC]; p++) {
    80001d5c:	00016997          	auipc	s3,0x16
    80001d60:	4a498993          	addi	s3,s3,1188 # 80018200 <tickslock>
  ...
  asm volatile("csrw sstatus, %0" : : "r" (x));
    ...
    80001dc0:	a4448493          	addi	s1,s1,-1468 # 80012800 <proc>
      if(p->state == RUNNABLE) {
...
0000000080001dc8 <sched>:
{
  ...
  if(mycpu()->noff != 1)
    ...
    80001dec:	5e870713          	addi	a4,a4,1512 # 800123d0 <pid_lock>
  ...
  intena = mycpu()->intena;
    ...
    80001e12:	5c290913          	addi	s2,s2,1474 # 800123d0 <pid_lock>
  ...
  swtch(&p->context, &mycpu()->context);
    ...
    80001e2a:	5e258593          	addi	a1,a1,1506 # 80012408 <cpus+0x8>
...
wakeup(void *chan)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001f12:	8f248493          	addi	s1,s1,-1806 # 80012800 <proc>
    if(p != myproc()){
      acquire(&p->lock);
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001f1e:	2e690913          	addi	s2,s2,742 # 80018200 <tickslock>
...
0000000080001f64 <reparent>:
{
  ...
  for(pp = proc; pp < &proc[NPROC]; pp++){
    ...
    80001f7a:	88a48493          	addi	s1,s1,-1910 # 80012800 <proc>
      pp->parent = initproc;
    80001f7e:	00008a17          	auipc	s4,0x8
    80001f82:	31aa0a13          	addi	s4,s4,794 # 8000a298 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80001f86:	00016997          	auipc	s3,0x16
    80001f8a:	27a98993          	addi	s3,s3,634 # 80018200 <tickslock>
...
0000000080001fba <exit>:
{
  ...
  if(p == initproc)
    ...
    80001fd6:	2c67b783          	ld	a5,710(a5) # 8000a298 <initproc>
  ...
  acquire(&wait_lock);
    ...
    8000201e:	3ce48493          	addi	s1,s1,974 # 800123e8 <wait_lock>
...
kill(int pid)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    80002070:	79448493          	addi	s1,s1,1940 # 80012800 <proc>
    80002074:	00016997          	auipc	s3,0x16
    80002078:	18c98993          	addi	s3,s3,396 # 80018200 <tickslock>
    acquire(&p->lock);
...
0000000080002110 <wait>:
{
  ...
  acquire(&wait_lock);
    ...
    80002134:	2b850513          	addi	a0,a0,696 # 800123e8 <wait_lock>
    ...
    80002146:	0be98993          	addi	s3,s3,190 # 80018200 <tickslock>
    sleep(p, &wait_lock);  //DOC: wait-sleep
    8000214a:	00010c17          	auipc	s8,0x10
    8000214e:	29ec0c13          	addi	s8,s8,670 # 800123e8 <wait_lock>
    ...
    80002180:	26c50513          	addi	a0,a0,620 # 800123e8 <wait_lock>
    ...
    800021ac:	24050513          	addi	a0,a0,576 # 800123e8 <wait_lock>
    ...
    800021f4:	61048493          	addi	s1,s1,1552 # 80012800 <proc>
    800021f8:	b7e1                	j	800021c0 <wait+0xb0>
      release(&wait_lock);
    800021fa:	00010517          	auipc	a0,0x10
    800021fe:	1ee50513          	addi	a0,a0,494 # 800123e8 <wait_lock>
...
procdump(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    800022c4:	69848493          	addi	s1,s1,1688 # 80012958 <proc+0x158>
    800022c8:	00016917          	auipc	s2,0x16
    800022cc:	09090913          	addi	s2,s2,144 # 80018358 <bcache+0x140>
    if(p->state == UNUSED)
      continue;
...
trapinit(void)
{
  ...
  initlock(&tickslock, "time");
    ...
    800023c0:	e4450513          	addi	a0,a0,-444 # 80018200 <tickslock>
...
clockintr()
{
  ...
  asm volatile("csrw 0x14d, %0" : : "r" (x));
    ...
    800024a8:	d5c48493          	addi	s1,s1,-676 # 80018200 <tickslock>
    800024ac:	8526                	mv	a0,s1
    800024ae:	f46fe0ef          	jal	80000bf4 <acquire>
    ticks++;
    800024b2:	00008517          	auipc	a0,0x8
    800024b6:	dee50513          	addi	a0,a0,-530 # 8000a2a0 <ticks>
...
sys_sleep(void)
{
  ...
  acquire(&tickslock);
    ...
    80002926:	8de50513          	addi	a0,a0,-1826 # 80018200 <tickslock>
    8000292a:	acafe0ef          	jal	80000bf4 <acquire>
  ticks0 = ticks;
    8000292e:	00008917          	auipc	s2,0x8
    80002932:	97292903          	lw	s2,-1678(s2) # 8000a2a0 <ticks>
  while(ticks - ticks0 < n){
    ...
    80002944:	8c098993          	addi	s3,s3,-1856 # 80018200 <tickslock>
    80002948:	00008497          	auipc	s1,0x8
    8000294c:	95848493          	addi	s1,s1,-1704 # 8000a2a0 <ticks>
    if(killed(myproc())){
  ...
  release(&tickslock);
    ...
    80002978:	88c50513          	addi	a0,a0,-1908 # 80018200 <tickslock>
  ...
  return 0;
    ...
    80002996:	86e50513          	addi	a0,a0,-1938 # 80018200 <tickslock>
...
sys_uptime(void)
{
  ...
  acquire(&tickslock);
    ...
    800029d6:	82e50513          	addi	a0,a0,-2002 # 80018200 <tickslock>
    800029da:	a1afe0ef          	jal	80000bf4 <acquire>
  xticks = ticks;
    800029de:	00008497          	auipc	s1,0x8
    800029e2:	8c24a483          	lw	s1,-1854(s1) # 8000a2a0 <ticks>
  release(&tickslock);
    800029e6:	00016517          	auipc	a0,0x16
    800029ea:	81a50513          	addi	a0,a0,-2022 # 80018200 <tickslock>
...
binit(void)
{
  ...
  initlock(&bcache.lock, "bcache");
    ...
    80002a1e:	7fe50513          	addi	a0,a0,2046 # 80018218 <bcache>
    80002a22:	952fe0ef          	jal	80000b74 <initlock>

  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
    80002a26:	0001d797          	auipc	a5,0x1d
    80002a2a:	7f278793          	addi	a5,a5,2034 # 80020218 <bcache+0x8000>
    80002a2e:	0001e717          	auipc	a4,0x1e
    80002a32:	a5270713          	addi	a4,a4,-1454 # 80020480 <bcache+0x8268>
    80002a36:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    80002a3a:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002a3e:	00015497          	auipc	s1,0x15
    80002a42:	7f248493          	addi	s1,s1,2034 # 80018230 <bcache+0x18>
...
bread(uint dev, uint blockno)
{
  ...
  acquire(&bcache.lock);
    ...
    80002a9e:	77e50513          	addi	a0,a0,1918 # 80018218 <bcache>
    80002aa2:	952fe0ef          	jal	80000bf4 <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    80002aa6:	0001e497          	auipc	s1,0x1e
    80002aaa:	a2a4b483          	ld	s1,-1494(s1) # 800204d0 <bcache+0x82b8>
    80002aae:	0001e797          	auipc	a5,0x1e
    80002ab2:	9d278793          	addi	a5,a5,-1582 # 80020480 <bcache+0x8268>
    ...
    80002ada:	74250513          	addi	a0,a0,1858 # 80018218 <bcache>
  ...
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    ...
    80002af0:	9dc4b483          	ld	s1,-1572(s1) # 800204c8 <bcache+0x82b0>
    80002af4:	0001e797          	auipc	a5,0x1e
    80002af8:	98c78793          	addi	a5,a5,-1652 # 80020480 <bcache+0x8268>
  ...
  panic("bget: no buffers");
    ...
    80002b2c:	6f050513          	addi	a0,a0,1776 # 80018218 <bcache>
...
brelse(struct buf *b)
{
  ...
  acquire(&bcache.lock);
    ...
    80002bb4:	66850513          	addi	a0,a0,1640 # 80018218 <bcache>
  ...
  if (b->refcnt == 0) {
    ...
    80002bd6:	64678793          	addi	a5,a5,1606 # 80020218 <bcache+0x8000>
    80002bda:	2b87b703          	ld	a4,696(a5)
    80002bde:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    80002be0:	0001e717          	auipc	a4,0x1e
    80002be4:	8a070713          	addi	a4,a4,-1888 # 80020480 <bcache+0x8268>
  ...
  release(&bcache.lock);
    ...
    80002bf8:	62450513          	addi	a0,a0,1572 # 80018218 <bcache>
...
bpin(struct buf *b) {
  ...
  acquire(&bcache.lock);
    ...
    80002c28:	5f450513          	addi	a0,a0,1524 # 80018218 <bcache>
  ...
  release(&bcache.lock);
    ...
    80002c3a:	5e250513          	addi	a0,a0,1506 # 80018218 <bcache>
...
bunpin(struct buf *b) {
  ...
  acquire(&bcache.lock);
    ...
    80002c5c:	5c050513          	addi	a0,a0,1472 # 80018218 <bcache>
  ...
  release(&bcache.lock);
    ...
    80002c6e:	5ae50513          	addi	a0,a0,1454 # 80018218 <bcache>
...
bfree(int dev, uint b)
{
  ...
  bp = bread(dev, BBLOCK(b, sb));
    ...
    80002c96:	c627a783          	lw	a5,-926(a5) # 800208f4 <sb+0x1c>
...
0000000080002cec <balloc>:
{
  ...
  for(b = 0; b < sb.size; b += BPB){
    ...
    80002cfa:	be67a783          	lw	a5,-1050(a5) # 800208dc <sb+0x4>
    ...
    80002d1a:	bc2b0b13          	addi	s6,s6,-1086 # 800208d8 <sb>
...
0000000080002ee0 <iget>:
{
  ...
  acquire(&itable.lock);
    ...
    80002ef8:	a0450513          	addi	a0,a0,-1532 # 800208f8 <itable>
    80002efc:	cf9fd0ef          	jal	80000bf4 <acquire>
  empty = 0;
    80002f00:	4901                	li	s2,0
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80002f02:	0001e497          	auipc	s1,0x1e
    80002f06:	a0e48493          	addi	s1,s1,-1522 # 80020910 <itable+0x18>
    80002f0a:	0001f697          	auipc	a3,0x1f
    80002f0e:	49668693          	addi	a3,a3,1174 # 800223a0 <log>
  ...
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    ...
    80002f3a:	9c250513          	addi	a0,a0,-1598 # 800208f8 <itable>
  ...
  release(&itable.lock);
    ...
    80002f66:	99650513          	addi	a0,a0,-1642 # 800208f8 <itable>
...
fsinit(int dev) {
  ...
  memmove(sb, bp->data, sizeof(*sb));
    ...
    80002fa8:	93498993          	addi	s3,s3,-1740 # 800208d8 <sb>
  ...
  initlog(dev, &sb);
    ...
    80002fd4:	90858593          	addi	a1,a1,-1784 # 800208d8 <sb>
...
0000000080002ff8 <iinit>:
{
  ...
  initlock(&itable.lock, "itable");
    ...
    80003012:	8ea50513          	addi	a0,a0,-1814 # 800208f8 <itable>
    80003016:	b5ffd0ef          	jal	80000b74 <initlock>
  for(i = 0; i < NINODE; i++) {
    8000301a:	0001e497          	auipc	s1,0x1e
    8000301e:	90648493          	addi	s1,s1,-1786 # 80020920 <itable+0x28>
    80003022:	0001f997          	auipc	s3,0x1f
    80003026:	38e98993          	addi	s3,s3,910 # 800223b0 <log+0x10>
...
0000000080003050 <ialloc>:
{
  ...
  for(inum = 1; inum < sb.ninodes; inum++){
    ...
    8000305c:	88c72703          	lw	a4,-1908(a4) # 800208e4 <sb+0xc>
    ...
    8000307c:	860a0a13          	addi	s4,s4,-1952 # 800208d8 <sb>
...
000000008000310c <iupdate>:
{
  ...
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    ...
    80003124:	7d05a583          	lw	a1,2000(a1) # 800208f0 <sb+0x18>
...
000000008000318a <idup>:
{
  ...
  acquire(&itable.lock);
    ...
    8000319a:	76250513          	addi	a0,a0,1890 # 800208f8 <itable>
  ...
  release(&itable.lock);
    ...
    800031ac:	75050513          	addi	a0,a0,1872 # 800208f8 <itable>
...
00000000800031c0 <ilock>:
{
  ...
  if(ip->valid == 0){
    ...
    80003202:	6f25a583          	lw	a1,1778(a1) # 800208f0 <sb+0x18>
...
0000000080003342 <iput>:
{
  ...
  acquire(&itable.lock);
    ...
    80003352:	5aa50513          	addi	a0,a0,1450 # 800208f8 <itable>
  ...
  release(&itable.lock);
    ...
    8000336c:	59050513          	addi	a0,a0,1424 # 800208f8 <itable>
  ...
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    ...
    80003398:	56450513          	addi	a0,a0,1380 # 800208f8 <itable>
    ...
    800033be:	53e50513          	addi	a0,a0,1342 # 800208f8 <itable>
...
write_head(void)
{
  ...
  struct buf *buf = bread(log.dev, log.start);
    ...
    800038dc:	ac890913          	addi	s2,s2,-1336 # 800223a0 <log>
  ...
  for (i = 0; i < log.lh.n; i++) {
    ...
    800038fc:	ad870713          	addi	a4,a4,-1320 # 800223d0 <log+0x30>
...
000000008000392a <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    ...
    8000392e:	aa27a783          	lw	a5,-1374(a5) # 800223cc <log+0x2c>
    80003932:	08f05f63          	blez	a5,800039d0 <install_trans+0xa6>
{
    ...
    80003950:	a84a8a93          	addi	s5,s5,-1404 # 800223d0 <log+0x30>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003954:	4a01                	li	s4,0
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003956:	0001f997          	auipc	s3,0x1f
    8000395a:	a4a98993          	addi	s3,s3,-1462 # 800223a0 <log>
...
00000000800039d2 <initlog>:
{
  ...
  initlock(&log.lock, "log");
    ...
    800039e8:	9bc48493          	addi	s1,s1,-1604 # 800223a0 <log>
  ...
  for (i = 0; i < log.lh.n; i++) {
    ...
    80003a1e:	9b670713          	addi	a4,a4,-1610 # 800223d0 <log+0x30>
...
recover_from_log(void)
{
  ...
  log.lh.n = 0;
    ...
    80003a40:	9807a823          	sw	zero,-1648(a5) # 800223cc <log+0x2c>
...
begin_op(void)
{
  ...
  acquire(&log.lock);
    ...
    80003a66:	93e50513          	addi	a0,a0,-1730 # 800223a0 <log>
    80003a6a:	98afd0ef          	jal	80000bf4 <acquire>
  while(1){
    if(log.committing){
    80003a6e:	0001f497          	auipc	s1,0x1f
    80003a72:	93248493          	addi	s1,s1,-1742 # 800223a0 <log>
      sleep(&log, &log.lock);
    ...
    80003aaa:	8fa50513          	addi	a0,a0,-1798 # 800223a0 <log>
...
end_op(void)
{
  ...
  acquire(&log.lock);
    ...
    80003ad0:	8d448493          	addi	s1,s1,-1836 # 800223a0 <log>
  ...
  if(log.outstanding == 0){
    ...
    80003af0:	8b448493          	addi	s1,s1,-1868 # 800223a0 <log>
...
commit()
{
  if (log.lh.n > 0) {
    ...
    80003b08:	89c48493          	addi	s1,s1,-1892 # 800223a0 <log>
    ...
    80003b3a:	86a48493          	addi	s1,s1,-1942 # 800223a0 <log>
  ...
  for (tail = 0; tail < log.lh.n; tail++) {
    ...
    80003b60:	874a8a93          	addi	s5,s5,-1932 # 800223d0 <log+0x30>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    80003b64:	0001fa17          	auipc	s4,0x1f
    80003b68:	83ca0a13          	addi	s4,s4,-1988 # 800223a0 <log>
  ...
  for (tail = 0; tail < log.lh.n; tail++) {
    ...
    log.lh.n = 0;
    80003bc6:	0001f797          	auipc	a5,0x1f
    80003bca:	8007a323          	sw	zero,-2042(a5) # 800223cc <log+0x2c>
...
log_write(struct buf *b)
{
  ...
  acquire(&log.lock);
    ...
    80003bec:	7b890913          	addi	s2,s2,1976 # 800223a0 <log>
  ...
  for (i = 0; i < log.lh.n; i++) {
    ...
    80003c26:	7ae70713          	addi	a4,a4,1966 # 800223d0 <log+0x30>
  for (i = 0; i < log.lh.n; i++) {
  ...
  log.lh.block[i] = b->blockno;
    ...
    80003c42:	76278793          	addi	a5,a5,1890 # 800223a0 <log>
  ...
  if (i == log.lh.n) {  // Add new block to log?
    ...
    80003c56:	74e70713          	addi	a4,a4,1870 # 800223a0 <log>
  ...
  log.lh.block[i] = b->blockno;
    ...
    80003c84:	72070713          	addi	a4,a4,1824 # 800223a0 <log>
  ...
  release(&log.lock);
    ...
    80003c96:	70e50513          	addi	a0,a0,1806 # 800223a0 <log>
...
fileinit(void)
{
  ...
  initlock(&ftable.lock, "ftable");
    ...
    80003dbc:	73050513          	addi	a0,a0,1840 # 800224e8 <ftable>
...
filealloc(void)
{
  ...
  acquire(&ftable.lock);
    ...
    80003dda:	71250513          	addi	a0,a0,1810 # 800224e8 <ftable>
    80003dde:	e17fc0ef          	jal	80000bf4 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80003de2:	0001e497          	auipc	s1,0x1e
    80003de6:	71e48493          	addi	s1,s1,1822 # 80022500 <ftable+0x18>
    80003dea:	0001f717          	auipc	a4,0x1f
    80003dee:	6b670713          	addi	a4,a4,1718 # 800234a0 <disk>
    if(f->ref == 0){
  ...
  release(&ftable.lock);
    ...
    80003e02:	6ea50513          	addi	a0,a0,1770 # 800224e8 <ftable>
  ...
  return 0;
    ...
    80003e16:	6d650513          	addi	a0,a0,1750 # 800224e8 <ftable>
...
filedup(struct file *f)
{
  ...
  acquire(&ftable.lock);
    ...
    80003e3a:	6b250513          	addi	a0,a0,1714 # 800224e8 <ftable>
  ...
  release(&ftable.lock);
    ...
    80003e50:	69c50513          	addi	a0,a0,1692 # 800224e8 <ftable>
...
fileclose(struct file *f)
{
  ...
  acquire(&ftable.lock);
    ...
    80003e80:	66c50513          	addi	a0,a0,1644 # 800224e8 <ftable>
  ...
  release(&ftable.lock);
    ...
    80003ebe:	62e50513          	addi	a0,a0,1582 # 800224e8 <ftable>
  ...
  if(ff.type == FD_PIPE){
    ...
    80003ef6:	5f650513          	addi	a0,a0,1526 # 800224e8 <ftable>
...
fileread(struct file *f, uint64 addr, int n)
{
  ...
  return r;
    ...
    80004018:	43470713          	addi	a4,a4,1076 # 80022448 <devsw>
    ...
    80004052:	10078b63          	beqz	a5,80004168 <filewrite+0x11a>
{
  ...
  if(f->type == FD_PIPE){
    ...
    800040ba:	39270713          	addi	a4,a4,914 # 80022448 <devsw>
...
exec(char *path, char **argv)
{
 ...
 bad:
  ...
  ustack[argc] = 0;
    ...
    800046ca:	f9078793          	addi	a5,a5,-112 # ffffffffffffef90 <end+0xffffffff7ffdb9b0>
...
free_desc(int i)
{
  ...
  if(disk.free[i])
    ...
    800052e4:	1c078793          	addi	a5,a5,448 # 800234a0 <disk>
  ...
  disk.desc[i].addr = 0;
    ...
    800052f8:	1ac78793          	addi	a5,a5,428 # 800234a0 <disk>
  ...
  wakeup(&disk.free[0]);
    ...
    80005320:	19c50513          	addi	a0,a0,412 # 800234b8 <disk+0x18>
...
0000000080005348 <virtio_disk_init>:
{
  ...
  initlock(&disk.vdisk_lock, "virtio_disk");
    ...
    80005360:	26c50513          	addi	a0,a0,620 # 800235c8 <disk+0x128>
  ...
  *R(VIRTIO_MMIO_STATUS) = status;
  ...
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    ...
    800053c8:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fdb17f>
  ...
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
  ...
  disk.desc = kalloc();
    ...
    8000541e:	08648493          	addi	s1,s1,134 # 800234a0 <disk>
  ...
  if(!disk.desc || !disk.avail || !disk.used)
    ...
    8000543c:	07073703          	ld	a4,112(a4) # 800234a8 <disk+0x8>
  ...
  memset(disk.avail, 0, PGSIZE);
    ...
    80005454:	05048493          	addi	s1,s1,80 # 800234a0 <disk>
...
virtio_disk_rw(struct buf *b, int write)
{
  ...
  acquire(&disk.vdisk_lock);
    ...
    80005570:	05c50513          	addi	a0,a0,92 # 800235c8 <disk+0x128>
  ...
  for(int i = 0; i < NUM; i++){
    ...
    80005580:	f24b0b13          	addi	s6,s6,-220 # 800234a0 <disk>
  for(int i = 0; i < 3; i++){
  ...
  while(1){
    ...
    8000558a:	042c0c13          	addi	s8,s8,66 # 800235c8 <disk+0x128>
  ...
  for(int i = 0; i < NUM; i++){
    ...
    800055ac:	ef870713          	addi	a4,a4,-264 # 800234a0 <disk>
  ...
  for(int i = 0; i < NUM; i++){
    ...
    800055e4:	ed850513          	addi	a0,a0,-296 # 800234b8 <disk+0x18>
  ...
  if(write)
    ...
    80005600:	ea478793          	addi	a5,a5,-348 # 800234a0 <disk>
  ...
  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
  ...
  while(b->disk == 1) {
    ...
    800056d6:	ef690913          	addi	s2,s2,-266 # 800235c8 <disk+0x128>
  while(b->disk == 1) {
  ...
  disk.info[idx[0]].b = 0;
    ...
    800056fe:	da678793          	addi	a5,a5,-602 # 800234a0 <disk>
    80005702:	97ba                	add	a5,a5,a4
    80005704:	0007b423          	sd	zero,8(a5)
    int flag = disk.desc[i].flags;
    80005708:	0001e997          	auipc	s3,0x1e
    8000570c:	d9898993          	addi	s3,s3,-616 # 800234a0 <disk>
  ...
  release(&disk.vdisk_lock);
    ...
    80005730:	e9c50513          	addi	a0,a0,-356 # 800235c8 <disk+0x128>
...
virtio_disk_intr()
{
  ...
  acquire(&disk.vdisk_lock);
    ...
    80005760:	d4448493          	addi	s1,s1,-700 # 800234a0 <disk>
    80005764:	0001e517          	auipc	a0,0x1e
    80005768:	e6450513          	addi	a0,a0,-412 # 800235c8 <disk+0x128>
  ...
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
  ...
  release(&disk.vdisk_lock);
    ...
    800057dc:	df050513          	addi	a0,a0,-528 # 800235c8 <disk+0x128>
...
int myfunction(char *str) {
    ...
    800057fc:	e406                	sd	ra,8(sp)
    800057fe:	e022                	sd	s0,0(sp)
    80005800:	0800                	addi	s0,sp,16
    80005802:	85aa                	mv	a1,a0
    printf("%s\n", str);    
    80005804:	00002517          	auipc	a0,0x2
    80005808:	81c50513          	addi	a0,a0,-2020 # 80007020 <etext+0x20>
    8000580c:	cb7fa0ef          	jal	800004c2 <printf>
    return 0xABCDABCD;
}
    80005810:	abcdb537          	lui	a0,0xabcdb
    80005814:	bcd50513          	addi	a0,a0,-1075 # ffffffffabcdabcd <end+0xffffffff2bcb75ed>
    80005818:	60a2                	ld	ra,8(sp)
    8000581a:	6402                	ld	s0,0(sp)
    8000581c:	0141                	addi	sp,sp,16
    8000581e:	8082                	ret

0000000080005820 <sys_myfunction>:

int sys_myfunction(void) {
    80005820:	7119                	addi	sp,sp,-128
    80005822:	fc86                	sd	ra,120(sp)
    80005824:	f8a2                	sd	s0,112(sp)
    80005826:	0100                	addi	s0,sp,128
    char str[100];
    //Decode argument using argstr
    if (argstr(0, str, 100) < 0) return -1;
    80005828:	06400613          	li	a2,100
    8000582c:	f8840593          	addi	a1,s0,-120
    80005830:	4501                	li	a0,0
    80005832:	f9bfc0ef          	jal	800027cc <argstr>
    80005836:	02054263          	bltz	a0,8000585a <sys_myfunction+0x3a>
    printf("%s\n", str);    
    8000583a:	f8840593          	addi	a1,s0,-120
    8000583e:	00001517          	auipc	a0,0x1
    80005842:	7e250513          	addi	a0,a0,2018 # 80007020 <etext+0x20>
    80005846:	c7dfa0ef          	jal	800004c2 <printf>
    return myfunction(str);
    8000584a:	abcdb537          	lui	a0,0xabcdb
    8000584e:	bcd50513          	addi	a0,a0,-1075 # ffffffffabcdabcd <end+0xffffffff2bcb75ed>
}
    80005852:	70e6                	ld	ra,120(sp)
    80005854:	7446                	ld	s0,112(sp)
    80005856:	6109                	addi	sp,sp,128
    80005858:	8082                	ret
    if (argstr(0, str, 100) < 0) return -1;
    8000585a:	557d                	li	a0,-1
    8000585c:	bfdd                	j	80005852 <sys_myfunction+0x32>

000000008000585e <sys_getppid>:
...
int sys_getppid(void){
    ...
    80005866:	87afc0ef          	jal	800018e0 <myproc>
    8000586a:	7d1c                	ld	a5,56(a0)
}
    8000586c:	5b88                	lw	a0,48(a5)
