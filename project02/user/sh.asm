...
getcmd(char *buf, int nbuf)
{
  ...
  write(2, "$ ", 2);
      ...
      16:	24e58593          	addi	a1,a1,590 # 1260 <thread_join+0x36>
      1a:	4509                	li	a0,2
...
panic(char *s)
{
  ...
  fprintf(2, "%s\n", s);
      ...
      58:	21458593          	addi	a1,a1,532 # 1268 <thread_join+0x3e>
      5c:	4509                	li	a0,2
...
fork1(void)
{
  ...
  return pid;
    ...
    panic("fork");
      ...
      86:	1ee50513          	addi	a0,a0,494 # 1270 <thread_join+0x46>
...
000000000000008e <runcmd>:
{
  ...
  switch(cmd->type){
      ...
      ae:	2f670713          	addi	a4,a4,758 # 13a0 <thread_join+0x176>
    ...
    panic("runcmd");
      ...
      c6:	1b650513          	addi	a0,a0,438 # 1278 <thread_join+0x4e>
    ...
    fprintf(2, "exec %s failed\n", ecmd->argv[0]);
      ...
      e0:	1a458593          	addi	a1,a1,420 # 1280 <thread_join+0x56>
      e4:	4509                	li	a0,2
  ...
  exit(0);
    ...
    runcmd(rcmd->cmd);
     ...
     114:	18058593          	addi	a1,a1,384 # 1290 <thread_join+0x66>
     118:	4509                	li	a0,2
    ...
    if(fork1() == 0){
     ...
     176:	12e50513          	addi	a0,a0,302 # 12a0 <thread_join+0x76>
...
parseredirs(struct cmd *cmd, char **ps, char *es)
{
  ...
  while(peek(ps, es, "<>")){
     ...
     4d0:	dfca8a93          	addi	s5,s5,-516 # 12c8 <thread_join+0x9e>
  ...
  while(peek(ps, es, "<>")){
     ...
     4e2:	dca50513          	addi	a0,a0,-566 # 12a8 <thread_join+0x7e>
...
parseexec(char **ps, char *es)
{
  ...
  if(peek(ps, es, "("))
     ...
     59e:	d3660613          	addi	a2,a2,-714 # 12d0 <thread_join+0xa6>
  ...
  while(!peek(ps, es, "|)&;")){
     ...
     5ce:	d26b0b13          	addi	s6,s6,-730 # 12f0 <thread_join+0xc6>
  ...
  return ret;
     ...
     5f8:	ce450513          	addi	a0,a0,-796 # 12d8 <thread_join+0xae>
  ...
  while(!peek(ps, es, "|)&;")){
    ...
    if(argc >= MAXARGS)
     ...
     64a:	c9a50513          	addi	a0,a0,-870 # 12e0 <thread_join+0xb6>
...
000000000000066c <parsepipe>:
{
  ...
  if(peek(ps, es, "|")){
     ...
     688:	c7460613          	addi	a2,a2,-908 # 12f8 <thread_join+0xce>
     68c:	85ce                	mv	a1,s3
     68e:	854a                	mv	a0,s2
...
00000000000006c6 <parseline>:
{
  ...
  while(peek(ps, es, "&")){
     ...
     6e4:	c20a0a13          	addi	s4,s4,-992 # 1300 <thread_join+0xd6>
  ...
  if(peek(ps, es, ";")){
     ...
     70e:	bfe60613          	addi	a2,a2,-1026 # 1308 <thread_join+0xde>
     712:	85ce                	mv	a1,s3
     714:	854a                	mv	a0,s2
...
000000000000074e <parseblock>:
{
  ...
  if(!peek(ps, es, "("))
     ...
     764:	b7060613          	addi	a2,a2,-1168 # 12d0 <thread_join+0xa6>
  ...
  if(!peek(ps, es, ")"))
     ...
     788:	b9c60613          	addi	a2,a2,-1124 # 1320 <thread_join+0xf6>
     78c:	85ca                	mv	a1,s2
     78e:	8526                	mv	a0,s1
  ...
  cmd = parseredirs(cmd, ps, es);
    ...
    panic("parseblock");
     ...
     7be:	b5650513          	addi	a0,a0,-1194 # 1310 <thread_join+0xe6>
     7c2:	889ff0ef          	jal	4a <panic>
    panic("syntax - missing )");
     7c6:	00001517          	auipc	a0,0x1
     7ca:	b6250513          	addi	a0,a0,-1182 # 1328 <thread_join+0xfe>
...
nulterminate(struct cmd *cmd)
{
  ...
  switch(cmd->type){
     ...
     7f2:	bca70713          	addi	a4,a4,-1078 # 13b8 <thread_join+0x18e>
...
0000000000000852 <parsecmd>:
{
  ...
  peek(&s, es, "");
     ...
     87e:	b0e60613          	addi	a2,a2,-1266 # 1388 <thread_join+0x15e>
     882:	85a6                	mv	a1,s1
  ...
  nulterminate(cmd);
    ...
    fprintf(2, "leftovers: %s\n", s);
     ...
     8ac:	a9858593          	addi	a1,a1,-1384 # 1340 <thread_join+0x116>
     8b0:	4509                	li	a0,2
     8b2:	748000ef          	jal	ffa <fprintf>
    panic("syntax");
     8b6:	00001517          	auipc	a0,0x1
     8ba:	a2250513          	addi	a0,a0,-1502 # 12d8 <thread_join+0xae>
...
00000000000008c2 <main>:
{
  ...
  while((fd = open("console", O_RDWR)) >= 0){
     ...
     8d6:	a7e48493          	addi	s1,s1,-1410 # 1350 <thread_join+0x126>
     8da:	4589                	li	a1,2
     8dc:	8526                	mv	a0,s1
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
     ...
     970:	9ec58593          	addi	a1,a1,-1556 # 1358 <thread_join+0x12e>
     974:	4509                	li	a0,2
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    buf[i++] = digits[x % base];
     ...
     cec:	6e850513          	addi	a0,a0,1768 # 13d0 <digits>
     cf0:	883a                	mv	a6,a4
...
vprintf(int fd, const char *fmt, va_list ap)
{
  ...
  putc(fd, 'x');
    ...
    putc(fd, digits[x >> (sizeof(uint64) * 8 - 4)]);
     ...
     f82:	452b8b93          	addi	s7,s7,1106 # 13d0 <digits>
  ...
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
     ...
     fd0:	39c90913          	addi	s2,s2,924 # 1368 <thread_join+0x13e>
        for(; *s; s++)
...
int thread_create(void (*start_routine)(void *, void *), void *arg1, void *arg2)
{
    ...
    11d6:	ec26                	sd	s1,24(sp)
    11d8:	e84a                	sd	s2,16(sp)
    11da:	e44e                	sd	s3,8(sp)
  ...
  if (stack == 0) {
    11ea:	c105                	beqz	a0,120a <thread_create+0x3a>
    11ec:	86aa                	mv	a3,a0
  ...
  int tid = clone(start_routine, arg1, arg2, stack);
    ...
    11f0:	85ca                	mv	a1,s2
    11f2:	8526                	mv	a0,s1
    11f4:	aa9ff0ef          	jal	c9c <clone>

  if (tid < 0) {
    11f8:	02054163          	bltz	a0,121a <thread_create+0x4a>
    printf("clone failed\n");
    return -1;
  }
  ...
  return tid;
    ...
    11fe:	7402                	ld	s0,32(sp)
    1200:	64e2                	ld	s1,24(sp)
    1202:	6942                	ld	s2,16(sp)
    1204:	69a2                	ld	s3,8(sp)
    1206:	6145                	addi	sp,sp,48
    1208:	8082                	ret
    printf("stack allocation failed\n");
    120a:	00000517          	auipc	a0,0x0
    120e:	16650513          	addi	a0,a0,358 # 1370 <thread_join+0x146>
    1212:	e13ff0ef          	jal	1024 <printf>
    return -1;
    1216:	557d                	li	a0,-1
    1218:	b7d5                	j	11fc <thread_create+0x2c>
    printf("clone failed\n");
    121a:	00000517          	auipc	a0,0x0
    121e:	17650513          	addi	a0,a0,374 # 1390 <thread_join+0x166>
    1222:	e03ff0ef          	jal	1024 <printf>
    return -1;
    1226:	557d                	li	a0,-1
    1228:	bfd1                	j	11fc <thread_create+0x2c>

000000000000122a <thread_join>:
...
int thread_join(){
    ...
    1232:	1800                	addi	s0,sp,48
  //printf("thread join executed\n");
  void *stack;

  int pid = join(&stack);
    1234:	fd840513          	addi	a0,s0,-40
    1238:	a6dff0ef          	jal	ca4 <join>

  if (pid < 0)
    123c:	00054d63          	bltz	a0,1256 <thread_join+0x2c>
    1240:	84aa                	mv	s1,a0
    return -1;

  free(stack);
    1242:	fd843503          	ld	a0,-40(s0)
    1246:	e11ff0ef          	jal	1056 <free>

  return pid;
    124a:	8526                	mv	a0,s1
    124c:	70a2                	ld	ra,40(sp)
    124e:	7402                	ld	s0,32(sp)
    1250:	64e2                	ld	s1,24(sp)
    1252:	6145                	addi	sp,sp,48
    1254:	8082                	ret
    return -1;
    1256:	54fd                	li	s1,-1
    1258:	bfcd                	j	124a <thread_join+0x20>
